...
import patient
from parameters import ControllerType, dt_ctr
...
def control(
    ...
    *,
    ...
    if params["controller_type"] == ControllerType.FBL_RBF_2:
        ...
        fbl[idx] = -(1 / b_hat) * (
            -f_hat + G_d_p - params["lambda"] * (G_s - G_d)
        )

    if params["controller_type"] == ControllerType.FBL:
        u = fbl[idx]
    elif params["controller_type"] == ControllerType.FBL_RBF:
        d_hat[idx] = rbf(s, w, idx, t, params=params)
        u = fbl[idx] - (1 / b_hat) * (-d_hat[idx])
    elif params["controller_type"] == ControllerType.FBL_RBF_2:
        d_hat[idx] = rbf(s, w, idx, t, params=params)
        u = fbl[idx] - (1 / b_hat) * (-d_hat[idx])
