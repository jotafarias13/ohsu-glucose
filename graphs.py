...
def plot_exercise(data: dict, output_file: Path) -> None:
...
def plot_weights(data: dict, output_file: Path) -> None:
    _, ax = plt.subplots(nrows=1, ncols=1, figsize=(16, 8))
...
def plot_weights_norm(data: dict, output_file: Path) -> None:
    _, ax = plt.subplots(nrows=1, ncols=1, figsize=(16, 8))
...
def plot_rbf(params: dict, output_file: Path) -> None:
    error = np.arange(-0.1, 1.0, 0.0001, dtype=float)
    phi = [
        np.exp(
            -(1.0 / 2.0)
...
def plot_estimation(data: dict, output_file: Path, patient: Patient) -> None:
...
def run(data: dict, params: dict) -> None:
    ...
    plot_exercise(data, graphs_dir / graph_file)

    # neural network
    graph_file = "weights.pdf"
    plot_weights(data, graphs_dir / graph_file)

    # neural network weights norm
    graph_file = "weights-norm.pdf"
    plot_weights_norm(data, graphs_dir / graph_file)
    ...
    plot_estimation(data, graphs_dir / graph_file, patient)
